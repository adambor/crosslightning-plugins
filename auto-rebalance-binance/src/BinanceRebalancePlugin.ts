...
import {Binance, BinanceError} from "./binance/Binance";
...
export class BinanceRebalancePlugin<T extends SwapData> implements IPlugin<T> {

    name: string = "Binance rebalancing plugin";
    ...
    constructor(
        ...
        this.tokenAddresses.SOL = "So11111111111111111111111111111111111111112";
    ...
    async check() {
        ...
        if(this.state.state===RebalancingState.TRIGGERED) {
            if(this.state.srcToken==="BTC" || this.state.srcToken==="BTC-LN") {
                ...
                const destinationAddress = await this.binanceApi.getDepositAddress(this.state.srcToken, null, this.state.amountOut);
                ...
                const withdrawTxs = await this.swapContract.txsWithdraw(this.swapContract.toTokenAddress(this.state.srcTokenAddress), this.state.amountOut);
        ...
        if(this.state.state===RebalancingState.SC_WITHDRAWAL_CONFIRMED) {
            const destinationAddress = await this.binanceApi.getDepositAddress(this.state.srcToken, this.okxSmartChainName);
            console.log("[Rebalancing plugin: SC_WITHDRAWAL_CONFIRMED] Destination address: ", destinationAddress);
            const transferTxs = await this.swapContract.txsTransfer(this.swapContract.toTokenAddress(this.state.srcTokenAddress), this.state.amountOut, destinationAddress);
            this.state.outTxs = {};
        ...
        if(this.state.state===RebalancingState.OUT_TX_CONFIRMED) {
            ...
            const depositState = await this.binanceApi.getDeposit(this.state.outTxId);
        ...
        if(this.state.state===RebalancingState.DEPOSIT_RECEIVED) {
            const clientOrderId = this.binanceApi.generateOrderId();
            this.state.clientOrderId = clientOrderId;
            this.state.cooldown = Date.now()+(5*1000);
            await this.setState(RebalancingState.TRADE_EXECUTING);

            try {
                await this.binanceApi.marketTrade(this.state.srcToken, this.state.dstToken, this.state.amountOut, clientOrderId);
            } catch (e) {
                console.error(e);
                //TODO: Schedule retry
            }
        ...
        if(this.state.state===RebalancingState.TRADE_EXECUTING) {
            let tradeData;
            try {
                tradeData = await this.binanceApi.getTrade(this.state.srcToken, this.state.dstToken, this.state.clientOrderId);
            } catch (e) {
                if(e instanceof BinanceError) {
                    const isOrderNotFound = e.errorCode==="-2013";
                    if(isOrderNotFound) {
            ...
            if(tradeData.state==="filled") {
                ...
                this.state.amountIn = await this.binanceApi.getBalance(this.state.dstToken);
        ...
        if(this.state.state===RebalancingState.TRADE_EXECUTED) {
            this.state.withdrawalFee = await this.binanceApi.getWithdrawalFee(this.state.dstToken, this.okxSmartChainName);
            ...
            try {
                await this.binanceApi.withdraw(this.state.dstToken, this.okxSmartChainName, this.state.receivingAddress, withdrawalId, this.state.withdrawalFee, this.state.amountIn.sub(this.state.withdrawalFee));
            } catch (e) {
                if(e instanceof BinanceError) {
                    this.state.retryAt = Date.now() + retryTime;
                    ...
                    await this.setState(RebalancingState.RETRYING);
                    if(e.errorCode==="-4060") {
                        //Withdraw lock, because deposit TX didn't get enough confirmations yet
                        console.log("Deposit TX doesn't have enough confirmations for withdrawal");
                        return;
                    }
                }
                console.error(e);
        ...
        if(this.state.state===RebalancingState.WITHDRAWING) {
            const withdrawal = await this.binanceApi.getWithdrawal(this.state.withdrawalId);
            if(withdrawal==null) {
                this.state.retryAt = Date.now()+retryTime;
                this.state.retryState = RebalancingState.TRADE_EXECUTED;
                await this.setState(RebalancingState.RETRYING);
        ...
        if(this.state.state===RebalancingState.IN_TX_CONFIRMED) {
            if(this.state.dstToken!=="BTC" && this.state.dstToken!=="BTC-LN") {
                //Deposit back to SC
                const depositTxs = await this.swapContract.txsDeposit(this.swapContract.toTokenAddress(this.state.dstTokenAddress), this.state.amountIn.sub(this.state.withdrawalFee));
    ...
    async onEnable(
        ...
        this.binanceApi = new Binance(this.apiKey, this.apiSecret);
        ...
        // this.state.amountOut = new BN("1000000");
        ...
        //
        // const result = await this.binanceApi.withdraw("BTC-LN", null, addr, "Lorem", null, null);
        //
        // console.log(result);

        // const apiResult = await binanceApi.call({
        //     method: 'POST',
        ...
        // const apiResult = await this.binanceApi.getWithdrawalFee("USDC", "Polygon", new BN(10000000));
        //
